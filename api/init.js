...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ success: true, message: "Database initialized successfully", version: "v3-cascade" });
  } catch (err) {
    return res.status(500).json({ error: err.message, version: "v3-cascade" });
  }
}
